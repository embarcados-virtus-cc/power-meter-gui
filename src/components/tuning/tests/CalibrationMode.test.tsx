...
describe('CalibrationMode', () => {
    ...
    it('calls onModeChange when clicking external', () => {
        ...
        render(<CalibrationMode activeMode="internal" onModeChange={onModeChange} />)
    ...
    it('displays status badges when status provided', () => {
        const status = { rx: true, tx: false, bias: true, voltage: true, temp: true }
        render(<CalibrationMode activeMode="internal" onModeChange={() => { }} status={status} />)
