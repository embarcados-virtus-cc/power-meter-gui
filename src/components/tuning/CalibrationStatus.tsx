        ...
        <div className="space-y-8">
            ...
            <Card className="bg-transparent border-zinc-800">
                <CardHeader>
                    ...
                    <p className="text-sm text-slate-400">Verificação dos valores de calibração atuais</p>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="mt-8 bg-green-500/10 border border-green-500/20 rounded-lg p-4 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-green-500/70 text-xs mt-1">
                                Todos os parâmetros estão dentro dos limites de tolerância especificados.
                            </p>
                        </div>
                    </div>
                </CardContent>
            ...
            <Card className="bg-transparent border-zinc-800">
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                        <div>
                            <Label className="text-xs text-slate-500 block mb-1">Tipo DDM</Label>
                            <div className="font-mono text-slate-200">{info.ddmType}</div>
                        </div>
                        <div>
                            <Label className="text-xs text-slate-500 block mb-1">Versão SFF-8472</Label>
                            <div className="font-mono text-slate-200">{info.version}</div>
                        </div>
                        <div>
                            <Label className="text-xs text-slate-500 block mb-1">RX Power Type</Label>
                            <div className="font-mono text-slate-200">{info.rxPowerType}</div>
                        </div>
                        <div>
                            <Label className="text-xs text-slate-500 block mb-1">Address Change</Label>
                            <div className="font-mono text-slate-200">{info.addressChange}</div>
                        </div>
                    </div>
    ...
    return (
        <div className="relative p-2">
            ...
            <div className="grid grid-cols-2 gap-8">
                <div>
                    <span className="text-[10px] uppercase text-slate-500 block">Lido:</span>
                    <span className="font-mono text-xs text-slate-300">{value}</span>
                </div>
                <div>
                    <span className="text-[10px] uppercase text-slate-500 block">Esperado:</span>
